...
class ContextStack:
    ...
    def init(self, name: str, attrs: dict, chunks: list[BlockChunk]):
        ...
        self.push(ctx_list)
        return [c.block for c in ctx_list]
    ...
    def commit(self, name: str, chunks: list[BlockChunk]):
        ...
        schema_ctx_list = self.pop()
        for schema_ctx in schema_ctx_list:
...
class XmlParser(Process):
    ...
    def _handle_start(self, name: str, attrs: dict, chunks: list[BlockChunk]):
        ...
        if name == self._root_tag:
            chunks = []
        events = self._ctx_stack.init(name, attrs, chunks)
        for event in events:
            self._emit_event("block_init", event)
    ...
    def _handle_end(self, name: str, chunks: list[BlockChunk]):
        ...
        if name == self._root_tag:
            chunks = []
        events = self._ctx_stack.commit(name, chunks)        
        for event in events:
            self._emit_event("block_commit", event)
    ...
    def _handle_chardata(self, chunks: list[BlockChunk]):
        ...
        # print(f"{self._ctx_stack.top().block.tags}:", output)
        for out in output:
            if out:
                if isinstance(out, Block):
                    self._emit_event("block", out)
                else:
                    self._emit_event("block_delta", out)
